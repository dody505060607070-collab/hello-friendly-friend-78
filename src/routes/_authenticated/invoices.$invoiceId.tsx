...
function InvoiceDetailPage() {
  ...
  const markPaid = useMutation({ mutationFn: async () => {
    ...
    if (amount <= 0) throw new Error("الفاتورة مدفوعة بالكامل");
    const insert = await supabase.from("invoice_payments").insert({ invoice_id: invoiceId, amount, paid_at: new Date().toISOString().slice(0, 10), method: "manual", reference: "تسجيل يدوي كمدفوعة" });
    if (insert.error) throw insert.error;
    const update = await supabase.from("invoices").update({ status: "paid" }).eq("id", invoiceId);
    if (update.error) throw update.error;
  }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["invoice-detail", invoiceId] }); queryClient.invalidateQueries({ queryKey: ["invoices"] }); toast.success("تم تسجيل الفاتورة كمدفوعة بالكامل"); }, onError: (error) => toast.error(error instanceof Error ? error.message : "تعذّر تسجيل الفاتورة كمدفوعة") });
  const removeInvoice = useMutation({ mutationFn: async () => deleteInvoice(invoiceId), onSuccess: () => { toast.success("تم حذف الفاتورة"); queryClient.invalidateQueries({ queryKey: ["invoices"] }); navigate({ to: "/invoices" }); }, onError: (error) => toast.error(error instanceof Error ? error.message : "تعذّر حذف الفاتورة") });
  ...
  return <>
    ...
    <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title={`حذف الفاتورة ${data.invoice.invoice_number}`} subtitle="سيتم حذف جميع مدفوعات وبنود الفاتورة نهائيًا — لا يمكن التراجع." footer={<><PrimaryButton onClick={() => removeInvoice.mutate()} disabled={removeInvoice.isPending}>{removeInvoice.isPending ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}تأكيد الحذف</PrimaryButton><GhostButton onClick={() => setDeleteOpen(false)}>إلغاء</GhostButton></>}><p className="text-[13px] text-muted-foreground">هل أنت متأكد من حذف هذه الفاتورة نهائيًا؟</p></Modal>
  </>;
}
